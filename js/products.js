...
export async function getProducts(){
  try{
    ...
    console.error("Could not read products from Firestore:", e);
    return [];
  }
}
...
/* ---------- Product photos: no Firebase Storage needed.
   The image is resized/compressed in the browser and stored as a
   base64 data URL directly on the product document — stays on the
   free Firestore plan, no Storage/billing upgrade required.
   Firestore's 1MB-per-document limit is why this stays small
   (maxWidth 700px, JPEG quality ~0.72 → usually 50-150KB). ---------- */
export function fileToCompressedDataUrl(file, maxWidth = 700, quality = 0.72){
