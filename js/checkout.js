...
export async function placeOrder(cart, products, customer){
  ...
  // Customer contact/delivery info, collected in the cart drawer just
  // before checkout — this is what lets the studio actually reach the
  // buyer and deliver the order (cash on delivery).
  const customerInfo = {
  ...
  try{
    ...
    await fetch(FORMSPREE_ENDPOINT, {
      ...
      body: JSON.stringify({
        _subject: `New order #${docRef.id} — ${customerInfo.name || "unnamed customer"}`,
        order_id: docRef.id,
