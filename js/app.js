...
export function makeCartController(products){
  ...
  const api = {
    ...
    async checkout(){
      ...
      const missing = [];
      if(!customer.name) missing.push(nameEl);
      if(!customer.phone) missing.push(phoneEl);
      if(!customer.address) missing.push(addressEl);

      [nameEl, phoneEl, addressEl].forEach(el => el.classList.remove('field-error'));
      if(missing.length){
        missing.forEach(el => el.classList.add('field-error'));
        if(errorEl){
          errorEl.textContent = "Please fill in your name, phone, and delivery address so the studio can reach you.";
      ...
      try{
        await placeOrder(cart, products, customer);
        cart = getCart();
        render();
        nameEl.value = ""; phoneEl.value = ""; addressEl.value = "";
        alert("Order received — payment is cash on delivery. We'll contact you to confirm before it ships.");
      }catch(e){
        console.error("Could not save order:", e);
        alert("Something went wrong placing your order. Please try again, or reach the studio directly.");
      }
    },
  };
  window.__cart = api;
