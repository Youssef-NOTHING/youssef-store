...
const CHAT_QA = [
  ...
  { keys:["contact","email","whatsapp","reach","talk to","phone"],
    reply:"You can reach the studio at youssefboukhmiss44@gmail.com or WhatsApp +212 780-93-73-30, Mon–Sat 10:00–18:00. There's also a contact form on the Shipping & FAQ page." },
...
const CHAT_FALLBACK = "I don't have an answer for that yet — the fastest way is to email youssefboukhmiss44@gmail.com or use the contact form on the Shipping & FAQ page.";
  ...
  const api = {
    ...
    async checkout(){
      ...
      if(!name || !phone || !address){
        if(errEl){
          errEl.textContent = "Please fill in your name, phone and delivery address.";
          errEl.style.display = "block";
        }
        (!name ? nameEl : !phone ? phoneEl : addressEl).focus();
        return;
      }
      if(errEl) errEl.style.display = "none";

      try{
        const result = await placeOrder(cart, products, { name, phone, address });
        cart = getCart();
        render();
        nameEl.value = ""; phoneEl.value = ""; addressEl.value = "";
        if(result.saved){
          alert(`Order received, ${name} — it'll be prepared and you pay in cash when it's delivered to:\n${address}\nWe'll call ${phone} to confirm.`);
        } else {
          alert(`Order received, ${name} — our system had a hiccup saving it, but the studio has been emailed your order directly and will call ${phone} to confirm.`);
        }
      }catch(e){
        console.error("Could not place order:", e);
        alert("Could not place the order — please check your connection and try again.");
      }
    },
  };
  window.__cart = api;
