...
import {
  ...
  uploadProductImage, fmtPrice, ICONS,
} from "./products.js";
...
window.showTab = function(tab){
  ...
  document.getElementById('ordersView').style.display = isProducts ? 'none' : 'block';
  document.getElementById('tabProducts').classList.toggle('active', isProducts);
  document.getElementById('tabOrders').classList.toggle('active', !isProducts);
...
async function loadOrders(){
  ...
  try{
    ...
    listEl.innerHTML = `<div class="dash-user">Could not load orders — check the console / Firestore rules.</div>`;
    return;
  }
  renderOrders();
}
